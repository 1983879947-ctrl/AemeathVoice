...
def get_spepc(hps, filename, dtype, device, is_v2pro=False):
    ...
    # 替换 torchaudio.load 为 soundfile（避免 torchcodec DLL 缺失）
    import soundfile as sf
    audio_data, sr0 = sf.read(filename)
    import torch as _torch
    audio = _torch.FloatTensor(audio_data).unsqueeze(0)  # [channels, samples]
    if sr0 != sr1:
...
def get_tts_wav(
    ...
    for i_text, text in enumerate(texts):
        ...
        if model_version not in v3v4set:
            ...
            _audio_data, sr = _sf.read(ref_wav_path)
            import torch as _torch2
            ref_audio = _torch2.FloatTensor(_audio_data).unsqueeze(0).to(device).float()
